...
def py_verify(msg_hash: int, r: int, s: int, public_key: Union[int, ECPoint]) -> bool:
    ...
    if isinstance(public_key, int):
        ...
        try:
            ...
            return False
        assert pow(y, 2, FIELD_PRIME) == (pow(public_key, 3, FIELD_PRIME) + ALPHA * public_key + BETA) % FIELD_PRIME
        return py_verify(msg_hash, r, s, (public_key, y)) or py_verify(msg_hash, r, s, (public_key, (-y) % FIELD_PRIME))
    else:
