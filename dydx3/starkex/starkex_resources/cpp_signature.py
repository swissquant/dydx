...
def get_cpp_lib(crypto_c_exports_path):
    ...
    CPP_LIB_PATH.Hash.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
    CPP_LIB_PATH.Verify.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
    CPP_LIB_PATH.Verify.restype = bool
    CPP_LIB_PATH.Sign.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
...
def cpp_hash(left, right) -> int:
    ...
    if (
        CPP_LIB_PATH.Hash(left.to_bytes(32, "little", signed=False), right.to_bytes(32, "little", signed=False), res)
        != 0
    ):
