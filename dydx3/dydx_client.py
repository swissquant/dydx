    ...
    def __init__(
        ...
        if web3 is not None or web3_provider is not None:
            if isinstance(web3_provider, str):
                web3_provider = Web3.HTTPProvider(web3_provider, request_kwargs={"timeout": self.api_timeout})
        ...
        self.network_id = int(network_id or self.network_id or NETWORK_ID_MAINNET)
        ...
        if stark_private_key is not None:
            self.stark_public_key, self.stark_public_key_y_coordinate = private_key_to_public_key_pair_hex(
                stark_private_key
            )
        ...
        if self.eth_signer and self.default_address and not self.api_key_credentials:
            # This may involve a web3 call, so recover on failure.
            try:
                self.api_key_credentials = self.onboarding.recover_default_api_key_credentials(
    ...
    def private(self):
        ...
        if not self._private:
            if self.api_key_credentials:
                ...
                raise Exception(
                    "Private endpoints not supported " + "since api_key_credentials were not specified",
                )
