...
class Eth(object):
    ...
    def send_eth_transaction(
        ...
        if "gasPrice" not in options:
            try:
                options["gasPrice"] = self.web3.eth.gasPrice + DEFAULT_GAS_PRICE_ADDITION
            except Exception:
        ...
        try:
            ...
            while auto_detect_nonce and (
                "nonce too low" in str(error) or "replacement transaction underpriced" in str(error)
            ):
                try:
                    options["nonce"] += 1
                    signed = self.sign_tx(method, options)
    ...
    def get_next_nonce(
        ...
        if self._next_nonce_for_address.get(ethereum_address) is None:
            self._next_nonce_for_address[ethereum_address] = self.web3.eth.getTransactionCount(ethereum_address)
    ...
    def register_user(
        ...
        if ethereum_address is None:
            raise ValueError(
                "ethereum_address was not provided, " "and no default address is set",
            )
